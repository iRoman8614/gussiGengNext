                        ...
                        <div className={styles.list}>
                            ...
                            {activeIndex === 1 && <>
                                {limitLevels.length !== 0 ? <div className={styles.itemsList}>{limitLevels.map((item, index) => (
                                    <ItemPlaceholder img={limitImages[item.key]} name={limitNames[item.key]} need={index > 0 && limitNames[limitLevels[index-1].key]} item={item} key={index} onClick={() => openUpgradeModal(item)} available={isAvailable(item)} />
                                ))}</div> : <div className={styles.warning}>{t('EXP.noups')}</div>}
                            </>}
                        </div>
                    </div>
